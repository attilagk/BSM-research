...
TODO: idealized precision-recall figure
...
\node (D) at (1.2in,0in) {};
\draw (A) rectangle (C);
\path (A) -- node [rotate=90, anchor=south] {precision} (B);
\path (A) -- node [below] {recall} (D);
\end{tikzpicture}
\end{center}
\end{column}
...
\includegraphics[width=1\textwidth]{figures/from-others/vmc-fig1.png}
...
\begin{center}
labeled set \(j\): \(\{\overbrace{y_{i}}^\text{label},
\overbrace{\mathbf{x}_{ij}}^\text{annot}\}_{i=1,...,n_j}\)
{\small
