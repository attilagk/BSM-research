...
ASD GWAS (FDR below $\alpha=0.05$).  For four additional categories the
forward algorithm selected the disease status, Dx, variable
(Table~\ref{tab:modsel}) and the local rate was nonsignificantly increased
(Fig.~\ref{fig:testresults}) in schizophrenic samples; these are: Bipolar
...
\begin{table}
\begin{tabular}{lrlr}
group & SNV category & selected model formula & size \\
\hline
\hline
