...
Fig.~S\ref{fig:binomial-QQ},~S\ref{fig:poisson-QQ} from the progressive improvement in the
goodness of fit with increasing category size.  This analysis also showed that
for the smaller sized ASD GWAS (size 22) the fit is already adequate and for
the relatively large Schizophrenia GWAS (size 86) and Psychiatric GWAS (size
176) the fit is excellent, which gives further support to our model based
inference regarding these categories.
...
by narrowing down the gene set categories to likely damaging SNVs.  (We defined
likely damaging SNVs as those in either the coding nonsynonymous, stop-gain or
splice site category.  A complementary test for neutral SNVs was not
performed due to the difficulty of defining likely neutral mutations.)
However, our attempt fa
