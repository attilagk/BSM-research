...
# annotate using vcflib a single VCF read from STDIN
...
echo "Annotate $invcf with the number of bases to the closest variant."
...
echo "Annotate $invcf with the entropy of the reference sequence near the variant in a 7 bp window."
...
echo "Annotate $invcf with the entropy of the reference sequence near the variant in a 15 bp window."
...
echo "Left normalize $invcf"
bcftools norm -o $outvcf -O $outputType -f $REFERENCE -D $OUT3.gz

rm $OUT0.* $OUT1.* $OUT2.* $OUT3.* $outvcf.*
