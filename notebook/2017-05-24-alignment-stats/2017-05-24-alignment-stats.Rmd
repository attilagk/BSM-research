...
```{r read-depth-genome-log, echo=FALSE, fig.asp=1.5}
scalingf <- 1e6
smalldata <- FALSE
rd.g <- if(smalldata) rd.genome.100 else rd.genome
depth.plot(rd.g, chromosomes, scales = list(y = list(log = TRUE)))
```

```{r read-depth-genome, echo=FALSE, fig.asp=1.5}
depth.plot(rd.g, chromosomes, ylim = c(0, 250))
```

```{r read-depth-repres, echo=FALSE, fig.asp=0.5}
