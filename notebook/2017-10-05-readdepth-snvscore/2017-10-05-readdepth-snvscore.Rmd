...
source("2017-10-05-readdepth-snvscore.R")
```
...
A relatively small fraction (`r nrow(subset(df$seg32MB, subset = FILTER == "PASS")) / nrow(df$seg32MB)`) of calls have `FILTER = PASS` value.  The histograms of the corresponding LOD distributions (below) are therefore noisier than the ones above.  As much as the noise allows comparison, the LOD distribution for the two subhypotheses (mosaic muscle and mosaic NeuN+) are similar for `PASS`ing calls at all subsampling frequencies.
